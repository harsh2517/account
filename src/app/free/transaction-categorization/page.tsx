...
import { Eye, Landmark,  Upload, Trash2, FileDown, Edit3, AlertTriangle, ArrowUp, ArrowDown, Search, Sparkles, SearchCheck,  PlusCircle, Info } from "lucide-react";
...
import DownloadableFile from "@/components/ui/downloadable-file";
...
const [isViewHistoricalRefDialogOpen, setIsViewHistoricalRefDialogOpen] = useState(false);
const [isViewCoaDialogOpen, setIsViewCoaDialogOpen] = useState(false);
...
//View historical references
const ViewHistoricalReferencesDialog = () => (
  <Dialog open={isViewHistoricalRefDialogOpen} onOpenChange={setIsViewHistoricalRefDialogOpen}>
    <DialogContent className="sm:max-w-4xl max-h-[80vh]">
      <DialogHeader>
        <DialogTitle>Historical Reference Data</DialogTitle>
        <DialogDescription>
          Viewing {historicalReferences.length} historical reference entries.
        </DialogDescription>
      </DialogHeader>
      <ScrollArea className="max-h-[60vh]">
        <Table>
          ...
          <TableBody>
            {historicalReferences.map((ref) => (
              <TableRow key={ref.id}>
                <TableCell>{ref.keyword}</TableCell>
                <TableCell>{ref.vendorCustomerName}</TableCell>
                <TableCell>{ref.glAccount}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
...
//View chat of accounts
const ViewChartOfAccountsDialog = () => (
  <Dialog open={isViewCoaDialogOpen} onOpenChange={setIsViewCoaDialogOpen}>
    <DialogContent className="sm:max-w-4xl max-h-[80vh]">
      <DialogHeader>
        <DialogTitle>Chart of Accounts Data</DialogTitle>
        <DialogDescription>
          Viewing {chartOfAccounts.length} chart of accounts entries.
        </DialogDescription>
      </DialogHeader>
      <ScrollArea className="max-h-[60vh]">
        <Table>
  ...
  return (
      ...
      <header className="mb-4 md:mb-8 flex flex-col md:flex-row items-center justify-between">
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4">
              <h4 className="font-medium mb-2">Sample Files (Download to get started):</h4>
              <div className="space-y-2">
                <DownloadableFile 
                  fileName="sample_transactions.xlsx"
                  filePath="/samples/sample_transactions.xlsx"
                  description="Example bank transaction data with proper formatting"
                />
                <DownloadableFile 
                  fileName="sample_historical_refs.xlsx"
                  filePath="/samples/sample_historical_refs.xlsx"
                  description="Sample historical reference data for categorization"
                />
                <DownloadableFile
                  fileName="sample_chart_of_accounts.xlsx"
                  filePath="/samples/sample_chart_of_accounts.xlsx"
                  description="Example chart of accounts structure"
                />
              </div>
            </div>
    ...
    <Button
      variant="ghost"
      size="icon"
      onClick={() => setIsViewHistoricalRefDialogOpen(true)}
      className="h-6 w-6 mr-1"
      disabled={commonButtonDisabled || historicalReferences.length === 0}
      title="View historical reference data"
    >
    ...
    <Button
      ...
      className="h-6 w-6"
      disabled={commonButtonDisabled}
      title="Remove historical reference file"
    >
    ...
    <Dialog open={isUploadHistoricalRefDialogOpen} onOpenChange={(isOpen) => {
     ...
     <Button
       variant="ghost"
       size="icon"
       onClick={() => setIsViewCoaDialogOpen(true)}
       className="h-6 w-6 mr-1"
       disabled={commonButtonDisabled || chartOfAccounts.length === 0}
       title="View chart of accounts data"
     >
       <Eye className="h-3 w-3" />
     </Button>
     ...
     <Button
       ...
       className="h-6 w-6"
       disabled={commonButtonDisabled}
       title="Remove chart of accounts file"
     >
       <Trash2 className="h-3 w-3" />
     </Button>
   </div>
...
<ViewHistoricalReferencesDialog />
    <ViewChartOfAccountsDialog />
    </div>
  );
}
