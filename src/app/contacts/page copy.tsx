...
export default function ContactsPage() {
  ...
  const handleSaveContact: SubmitHandler<ContactFormValues> = async (data) => {
    ...
    try {
      if (editingContact) {
        ...
        await addDoc(contactRef, { ...contactData, createdAt: serverTimestamp() });
  ...
  const handleSaveBulkEdit = async () => {
    if (!user || selectedContactIds.length === 0 || !bulkEditType) {
    ...
    try {
        const batch = writeBatch(db);
        selectedContactIds.forEach(id => {
            batch.update(doc(db, "contacts", id), { type: bulkEditType });
        });
        await batch.commit();
  ...
  const handleImportData = async () => {
    if (!user || !columnMappings.name || !columnMappings.type) {
    ...
    try {
      excelData.forEach(row => {
        ...
        const newContact: Omit<ContactItem, 'id' | 'createdAt'> = {
          userId: user.uid,
          name,
