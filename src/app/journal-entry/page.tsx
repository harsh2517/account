...
const deleteJournalEntryLineFromLedger = async (companyId: string, lineId: string, batch?: WriteBatch) => {
  const ledgerRef = collection(db, "all_transactions_ledger");
...
export default function JournalEntryPage() {
  ...
  const fetchJournalEntryLines = useCallback(async () => {
    ...
    setIsFetching(true);
    try {
      const q = query(collection(db, "journal_entry_lines"), where("companyId", "==", selectedCompanyId));
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
        ...
        snapshot.forEach(docSnap => fetchedContacts.push({
          id: docSnap.id, 
          ...(docSnap.data() as Omit<PageContactItem, 'id'>)
        }));
  ...
  }, [user, selectedCompanyId, fetchJournalEntryLines, fetchChartOfAccounts, fetchContacts]);
  ...
  const handleConfirmBulkDelete = async () => {
    if (!user || !selectedCompanyId || selectedEntryLineIds.length === 0) return;
    setIsLoading(true);
    try {
      ...
      for (const lineId of selectedEntryLineIds) {
        ...
        if (lineToDelete?.isLedgerApproved) {
          await deleteJournalEntryLineFromLedger(selectedCompanyId, lineId, batch);
        ...
        await logAction(
          ...
          "Journal Entries",
          [`Deleted ${selectedEntryLineIds.length} journal entry lines`]
        );
      }
      await batch.commit();
  ...
  const handleConfirmBulkPostToLedger = async () => {
    ...
    try {
      ...
      for (const line of linesToPost) {
        ...
        addJournalEntryLineToLedger(batch, selectedCompanyId, user.uid, line, contactTypeForLedger);
      ...
      if (postedCount > 0 || contactsCreatedCount > 0) {
        ...
        await logAction(
          ...
          "Journal Entries",
          [
            `Posted ${postedCount} lines to ledger`,
            contactsCreatedCount > 0 ? `Created ${contactsCreatedCount} contacts` : null,
            postingErrors.length > 0 ? `${postingErrors.length} lines skipped` : null
  ...
  const handleConfirmBulkUnpostFromLedger = async () => {
    ...
    try {
      ...
      for (const line of linesToUnpost) {
        await deleteJournalEntryLineFromLedger(selectedCompanyId, line.id, batch);
  ...
  const handleSaveNewJournalEntry = async () => {
    ...
    try {
      ...
      for (const line of newEntryLines) {
        ...
        batch.set(doc(linesCollectionRef), lineData);
      }
    }
      await batch.commit();
      await logAction(
        ...
        "Journal Entries",
        [`Created journal entry with ${newEntryLines.length} lines`, `Journal Set ID: ${journalSetId}`]
      );
  ...
  const handleImportJournalEntries = async () => {
    ...
    const processGroup = async (rowsInGroup: any[][], groupIdentifier: string): Promise<boolean> => {
        ...
        linesForThisGroup.forEach(lineData => {
            ...
            batch.set(newLineDocRef, { ...lineData, journalSetId: systemJournalSetId, createdAt: serverTimestamp() });
    ...
    try {
      if (currentImportedLinesCount > 0 || autoCreatedContactsThisImport > 0){ 
        await batch.commit()
        await logAction(
          "Import Journal Entries",
          "Journal Entries",
          [
                  `Imported ${currentImportedJECount} journal entries`,
                  `Imported ${currentImportedLinesCount} lines`,
                  autoCreatedContactsThisImport > 0 ? `Created ${autoCreatedContactsThisImport} contacts` : null,
                  skippedLinesForImport.length > 0 ? `${skippedLinesForImport.length} lines skipped` : null
              ].filter(Boolean) as string[]
          );
      }
