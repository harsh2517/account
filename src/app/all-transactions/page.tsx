...
export default function AllTransactionsPage() {
  ...
  const fetchLedgerItems = useCallback(async () => {
   if (!user || !selectedCompanyId) {
    ...
    setIsFetching(true);
    try {
      const q = query(collection(db, "all_transactions_ledger"),  where("companyId", "==", selectedCompanyId));
  ...
  const handleConfirmBulkDelete = async () => {
    if (!user || !selectedCompanyId || selectedEntryIds.length === 0) return;
  ...
  const handleExportToExcel = async () => {
    ...
    await logAction(
      ...
      [String(filteredAndSortedLedgerItems.length)]
    );
