...
import { ArrowLeft, FileText, PlusCircle, Upload, FileDown, Search, Edit3, Trash2, Library, AlertTriangle, CheckCircle2, AlertCircle, Calendar as CalendarIcon, DollarSign, Undo2, Download } from "lucide-react";
...
async function ensureCustomerExistsAndAddToBatch(
  ...
  if (!userId || !customerName || customerName.trim() === "" || customerName.trim() === "-") {
    return false;
  }
  ...
  const contactExists = existingContacts.some(
    (c) => c.name.trim().toLowerCase() === normalizedCustomerName && c.type === "Customer"
  );
  ...
  if (!contactExists) {
    ...
    existingContacts.push({ id: newContactRef.id, name: trimmedCustomerName, type: "Customer" });
    return true;
  }
  return false;
}

const addSalesInvoiceToLedger = (batch: WriteBatch, userId: string, invoice: SalesInvoice, accountsReceivableGL: string) => {
...
const deleteSalesInvoiceFromLedger = async (userId: string, invoiceId: string, batch?: WriteBatch) => {
...
export default function SalesInvoicesPage() {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    setIsFetching(true);
    try {
      const q = query(collection(db, "sales_invoices"), where("userId", "==", user.uid));
  ...
  const fetchChartOfAccounts = useCallback(async () => {
    if (!user) { setIsFetchingChartOfAccounts(false); return; }
  ...
  const fetchContacts = useCallback(async () => {
    if (!user) { setIsFetchingContacts(false); return; }
  ...
  }, [user, fetchInvoices, fetchChartOfAccounts, fetchContacts]);
  ...
  const handleSaveInvoice = async () => {
    ...
    const invoiceData: Omit<SalesInvoice, 'id' | 'createdAt' | 'paymentStatus' | 'paymentDate' | 'paidFromToBankGL'> = {
      userId: user.uid,
    ...
    try {
      ...
      if (!currentContactsForSave.find(c => c.name.trim().toLowerCase() === fullInvoiceData.customerName.toLowerCase() && c.type === "Customer")) {
         newCustomerCreated = await ensureCustomerExistsAndAddToBatch(user.uid, fullInvoiceData.customerName, currentContactsForSave, batch);
      }
      
      if (editingInvoice) {
        const invoiceRef = doc(db, "sales_invoices", editingInvoice.id);
        if (editingInvoice.isLedgerApproved) {
          await deleteSalesInvoiceFromLedger(user.uid, editingInvoice.id, batch);
  ...
  const handleDeleteInvoice = async (invoiceId: string) => {
    ...
    try {
      ...
      if (invoiceToDelete?.isLedgerApproved) {
        await deleteSalesInvoiceFromLedger(user.uid, invoiceId, batch);
  ...
  const handlePostInvoiceToLedger = async (invoiceId: string) => {
    ...
    try {
      const batch = writeBatch(db);
      addSalesInvoiceToLedger(batch, user.uid, invoiceToPost, accountsReceivableGL);
      batch.update(doc(db, "sales_invoices", invoiceId), { isLedgerApproved: true });
      await batch.commit();
  ...
  const handleUnpostInvoiceFromLedger = async (invoiceId: string) => {
    ...
    try {
        const batch = writeBatch(db);
        await deleteSalesInvoiceFromLedger(user.uid, invoiceId, batch);
        batch.update(doc(db, "sales_invoices", invoiceId), { isLedgerApproved: false });
        await batch.commit();
  ...
  const handleConfirmBulkPostToLedger = async () => {
    if (!user || selectedInvoiceIds.length === 0) return;
    setIsLoadingAction(true);
    ...
    try {
      ...
      for (const invoice of invoicesToPost) {
        const newCustomerCreated = await ensureCustomerExistsAndAddToBatch(user.uid, invoice.customerName, currentContactsForBulkPost, batch);
        if(newCustomerCreated) customerContactsCreated++;

        addSalesInvoiceToLedger(batch, user.uid, invoice, accountsReceivableGL);
        batch.update(doc(db, "sales_invoices", invoice.id), { isLedgerApproved: true });
        postedCount++;
      }
  ...
  const handleConfirmMarkAsPaid = async () => {
    if (!user || !paymentDateForDialog || !bankGLForPaymentDialog) {
  ...
  const handleDownloadInvoicePDF = (invoice: SalesInvoice) => {
    ...
    doc.setFontSize(20);
    doc.text(companyName || user?.displayName || 'Your Company', 20, currentY);
  ...
  return (
    <AuthGuard>
      <div className="container mx-auto px-4 py-8 animate-fade-in">
        ...
        <Dialog open={isCreateInvoiceDialogOpen} onOpenChange={(isOpen) => { setIsCreateInvoiceDialogOpen(isOpen); if (!isOpen) setEditingInvoice(null); }}>
          <DialogContent className="sm:max-w-3xl md:max-w-4xl lg:max-w-5xl h-[90vh] flex flex-col p-0">
            <DialogHeader className="p-6 pb-4 border-b"><DialogTitle className="font-headline text-2xl">{editingInvoice ? "Edit Sales Invoice" : "Create New Sales Invoice"}</DialogTitle><DialogDescription>Enter invoice details. Line items determine the total amount.</DialogDescription></DialogHeader>
            
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 flex-none">
                <div>
                    <Label htmlFor="invoiceDate">Invoice Date *</Label>
                    <Popover><PopoverTrigger asChild>
                        <Button id="invoiceDate" variant="outline" className={cn("w-full justify-start text-left font-normal mt-1", !invoiceDate && "text-muted-foreground")} disabled={isLoadingAction}>
                            <CalendarIcon className="mr-2 h-4 w-4" />{invoiceDate ? format(invoiceDate, "MM/dd/yyyy") : <span>Pick a date</span>}
                        </Button>
                ...
                <div>
                    ...
                    <Select value={customerName} onValueChange={setCustomerName} disabled={isLoadingAction || isFetchingContacts}>
                        <SelectTrigger id="customerName" className="mt-1"><SelectValue placeholder={isFetchingContacts ? "Loading..." : (customerContacts.length === 0 ? "No Customers" : "Select Customer")} /></SelectTrigger>
                        <SelectContent>
                            {customerContacts.map(c => <SelectItem key={c.id} value={c.name}>{c.name}</SelectItem>)}
                        </SelectContent>
                    </Select>
                     <Input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Or type new customer" className="mt-1 text-xs" disabled={isLoadingAction} />
                </div>
                <div>
                    <Label htmlFor="invoiceNumber">Invoice Number</Label>
                    <Input id="invoiceNumber" value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} placeholder="Optional" className="mt-1" disabled={isLoadingAction}/>
                </div>
                <div>
                    <Label htmlFor="invDueDate">Due Date</Label>
                    <Popover><PopoverTrigger asChild>
                        <Button id="invDueDate" variant="outline" className={cn("w-full justify-start text-left font-normal mt-1", !dueDate && "text-muted-foreground")} disabled={isLoadingAction}>
                            <CalendarIcon className="mr-2 h-4 w-4" />{dueDate ? format(dueDate, "MM/dd/yyyy") : <span>Optional</span>}
                        </Button>
                ...
                <div className="md:col-span-2 lg:col-span-4">
                    <Label htmlFor="invoiceDescription">General Description</Label>
                    <Textarea id="invoiceDescription" value={invoiceDescription} onChange={(e) => setInvoiceDescription(e.target.value)} placeholder="Optional overall description" className="mt-1" disabled={isLoadingAction}/>
                </div>
            </div>

            <div className="p-6 pt-0 border-b flex-none"><h4 className="text-md font-medium mb-2">Line Items (Total: ${totalInvoiceAmountFromLines.toFixed(2)})</h4></div>
            
            <div className="flex-grow overflow-hidden px-2">
                <ScrollArea className="h-full p-4 pb-0">
                    <div className="space-y-3">
                    {lineItems.map((line) => (
                        <Card key={line.localId} className="p-3 shadow-sm"><div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-start">
                            <div className="md:col-span-4"><Label htmlFor={`line-desc-${line.localId}`}>Description *</Label><Input id={`line-desc-${line.localId}`} value={line.description} onChange={(e) => handleLineItemChange(line.localId, 'description', e.target.value)} placeholder="Product/Service" className="mt-1 text-xs" disabled={isLoadingAction}/></div>
                            <div className="md:col-span-3"><Label htmlFor={`line-gl-${line.localId}`}>GL Account * (Revenue)</Label>
                                <Select value={line.glAccount} onValueChange={(val) => handleLineItemChange(line.localId, 'glAccount', val)} disabled={isLoadingAction || isFetchingChartOfAccounts || revenueGLAccounts.length === 0}>
                                    <SelectTrigger id={`line-gl-${line.localId}`} className="mt-1 text-xs"><SelectValue placeholder={isFetchingChartOfAccounts ? "Loading..." : (revenueGLAccounts.length === 0 ? "No Revenue GLs" : "Select GL")} /></SelectTrigger>
                                    <SelectContent>{revenueGLAccounts.map(acc => (<SelectItem key={acc.id} value={acc.glAccount}>{acc.glAccount}</SelectItem>))}</SelectContent>
                                </Select>
                            </div>
                            <div className="md:col-span-1"><Label htmlFor={`line-qty-${line.localId}`}>Qty *</Label><Input id={`line-qty-${line.localId}`} type="number" value={line.quantity} onChange={(e) => handleLineItemChange(line.localId, 'quantity', Number(e.target.value))} className="mt-1 text-xs text-right" disabled={isLoadingAction} min="0.01" step="0.01"/></div>
                            <div className="md:col-span-2"><Label htmlFor={`line-price-${line.localId}`}>Unit Price *</Label><Input id={`line-price-${line.localId}`} type="number" value={line.unitPrice} onChange={(e) => handleLineItemChange(line.localId, 'unitPrice', Number(e.target.value))} placeholder="0.00" className="mt-1 text-xs text-right" disabled={isLoadingAction} min="0" step="0.01"/></div>
            ...
            <DialogFooter className="p-6 pt-4 border-t flex-none">
              <Button variant="ghost" onClick={() => setIsCreateInvoiceDialogOpen(false)} disabled={isLoadingAction}>Cancel</Button>
              <Button onClick={handleSaveInvoice} disabled={isLoadingAction || !isDialogInvoiceFormComplete || (chartOfAccounts.length === 0 && !isFetchingChartOfAccounts) || (!accountsReceivableGL && !editingInvoice?.isLedgerApproved) }>
                {isLoadingAction ? <LoadingSpinner className="mr-2" /> : (editingInvoice ? "Update Invoice" : "Create Invoice")}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="font-headline flex items-center">
                        <Library className="mr-2 h-6 w-6 text-primary" /> Confirm Bulk Post to Ledger
                    </DialogTitle>
                    <DialogDescription>
                        You are about to post {selectedInvoiceIds.filter(id => !invoices.find(inv => inv.id === id)?.isLedgerApproved).length} selected pending invoice(s) to the ledger.
                        This will also attempt to auto-create any new customer contacts if they don't exist.
                        Are you sure you want to proceed?
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button variant="outline" onClick={() => setIsBulkPostDialogOpen(false)} disabled={isLoadingAction}>Cancel</Button>
                    <Button onClick={handleConfirmBulkPostToLedger} disabled={isLoadingAction || !accountsReceivableGL}>
                        {isLoadingAction && <LoadingSpinner className="mr-2" />}
        ...
        <Dialog open={isMarkAsPaidDialogOpen} onOpenChange={setIsMarkAsPaidDialogOpen}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="font-headline flex items-center">
                       <DollarSign className="mr-2 h-6 w-6 text-primary" /> Mark Invoice(s) as Paid
                    </DialogTitle>
                    <DialogDescription>
                        Select payment date and the bank account where payment was received for {selectedInvoiceIds.filter(id => invoices.find(i => i.id === id && i.paymentStatus !== "Paid")).length} invoice(s).
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div>
                        <Label htmlFor="paymentDateDialog">Payment Date *</Label>
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button id="paymentDateDialog" variant="outline" className={cn("w-full justify-start text-left font-normal mt-1", !paymentDateForDialog && "text-muted-foreground")} disabled={isLoadingAction}>
                                    <CalendarIcon className="mr-2 h-4 w-4" />{paymentDateForDialog ? format(paymentDateForDialog, "MM/dd/yyyy") : <span>Pick a date</span>}
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={paymentDateForDialog} onSelect={setPaymentDateForDialog} initialFocus /></PopoverContent>
                        </Popover>
                    </div>
                    <div>
                        <Label htmlFor="bankGLPaymentDialog">Bank GL Account (Received Into) *</Label>
                        <Select value={bankGLForPaymentDialog} onValueChange={setBankGLForPaymentDialog} disabled={isLoadingAction || isFetchingChartOfAccounts || bankAssetGLAccounts.length === 0}>
                            <SelectTrigger id="bankGLPaymentDialog" className="mt-1">
                                <SelectValue placeholder={isFetchingChartOfAccounts ? "Loading CoA..." : (bankAssetGLAccounts.length === 0 ? "No Asset GLs" : "Select Bank GL")} />
                            </SelectTrigger>
                            <SelectContent>
                                {bankAssetGLAccounts.map(acc => <SelectItem key={acc.id} value={acc.glAccount}>{acc.glAccount}</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                </div>
                <DialogFooter>
                    <Button variant="outline" onClick={() => setIsMarkAsPaidDialogOpen(false)} disabled={isLoadingAction}>Cancel</Button>
                    <Button onClick={handleConfirmMarkAsPaid} disabled={isLoadingAction || !paymentDateForDialog || !bankGLForPaymentDialog}>
                        {isLoadingAction && <LoadingSpinner className="mr-2" />}
                        Confirm Payment
                    </Button>
                </DialogFooter>
        ...
        <Dialog open={isMarkAsUnpaidDialogOpen} onOpenChange={setIsMarkAsUnpaidDialogOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Are you sure you want to mark this invoice as unpaid? This will reverse the payment from the ledger and delete the associated bank deposit transaction.</DialogDescription>
            </DialogHeader>
            <DialogFooter>
