...
import { format as dateFnsFormat, parse as dateFnsParse, isValid as isDateValid } from "date-fns";
...
const addBankTransactionToLedger = (
  ...
  const commonLedgerData = {
    ...
    const ledgerBase = { ...commonLedgerData, companyId: selectedCompanyId, customer: finalCustomer, vendor: finalVendor };
...
const deleteBankTransactionFromLedger = async (companyId: string, transactionId: string, batch?: WriteBatch) => {
    const ledgerRef = collection(db, "all_transactions_ledger");
...
export default function BankTransactionsPage() {
  ...
  }, [user, selectedCompanyId, fetchTransactions, fetchChartOfAccounts]);
  ...
  const handleImportData = async () => {
    ...
    setIsLoading(true);
    let importedCount = 0;
    const tempTransactionHolder: Array<Omit<Transaction, 'id' | 'createdAt' | 'isLedgerApproved'> & { 
      tempId: string;
      companyId: string;
      createdBy: string;
    }> = [];
    ...
    excelData.forEach((row, index) => {
      ...
      if (dateValue instanceof Date && isDateValid(dateValue)) {
        // Use formatInTimeZone to handle potential timezone issues correctly.
        formattedDateForStorage = formatInTimeZone(dateValue, 'UTC', 'yyyy-MM-dd');
        ...
        if (typeof dateValue === 'string') {
            ...
            // Add the number of days from Excel, subtracting 1 because Excel's epoch is day 1, not day 0.
            const d = new Date(excelEpoch.getTime() + (dateValue - 1) * 24 * 60 * 60 * 1000);
            if (isDateValid(d)) parsedJsDate = d;
        }
        
        if (parsedJsDate) {
            formattedDateForStorage = formatInTimeZone(parsedJsDate, 'UTC', "yyyy-MM-dd");
        } else {
      ...
      if (!formattedDateForStorage || !descriptionValue || !bankNameValue || !isDateValid(dateFnsParse(formattedDateForStorage, "yyyy-MM-dd", new Date()))) {
       ...
       const newTx: Omit<Transaction, 'id' | 'createdAt' | 'isLedgerApproved'> & {
      tempId: string;
      companyId: string ;
      createdBy: string;
    } = {
  ...
  if (tempTransactionHolder.length === 0) {
    ...
    try {
      ...
      await batch.commit();
      await logAction("IMPORT_TRANSACTIONS", "BankTransactions", ["date", "description", "amount"]);
  ...
  const handleDeleteTransaction = async (transactionId: string) => {
    ...
    try {
      ...
      if (transactionToDelete?.isLedgerApproved) {
        await deleteBankTransactionFromLedger(selectedCompanyId, transactionId, batch);
      }
      await batch.commit();
      await logAction("DELETE_TRANSACTION", "BankTransactions", ["transaction"]);
  ...
  const handleSaveInlineEdit = async () => {
    if (!editingRowId || !user || !selectedCompanyId) return;
    ...
    try {
      ...
      if (needsLedgerReset) {
        await deleteBankTransactionFromLedger(selectedCompanyId, editingRowId, batch);
      ...
      const changedFields = [];
      if (inlineEditVendor !== originalTransaction.vendor) changedFields.push("vendor");
      if (inlineEditGlAccount !== originalTransaction.glAccount) changedFields.push("glAccount");
      
      await logAction("UPDATE_TRANSACTION", "BankTransactions", changedFields);
  ...
  const handleExportToExcel = () => {
    ...
    const exportData = filteredAndSortedTransactions.map(t => ({
      Date: t.date ? dateFnsFormat(dateFnsParse(t.date, "yyyy-MM-dd", new Date()), "MM/dd/yyyy") : "",
        ...
        transactionsEligibleForAICategorization.forEach(originalTx => {
          ...
          if (suggestion) {
            ...
            modifiableTxsFromAI.push({
              ...
              createdBy: originalTx.createdBy,
            ...
            if(vendorChanged || glChanged) {
                ...
                if (Object.keys(updatePayload).length > 0) { 
                    if (tx.isLedgerApproved && (updatePayload.vendor || updatePayload.glAccount)) { 
                        await deleteBankTransactionFromLedger(selectedCompanyId, tx.id, batch);
    ...
    try {
      ...
      for (const dialogTx of approvedTransactionsFromDialog) {
        ...
        if (originalTx.isLedgerApproved) {
          await deleteBankTransactionFromLedger(selectedCompanyId, originalTx.id, batch);
          updatePayload.isLedgerApproved = false;
          ledgerResets++;
        } else {
          updatePayload.isLedgerApproved = false; 
      ...
      if (updatedCount > 0) {
        await logAction("AI_CATEGORIZE_TRANSACTIONS", "BankTransactions", ["vendor", "glAccount"]);
         await batch.commit();
      }
  ...
  const handleSaveBulkEdit = async () => {
    if (!user || !selectedCompanyId || selectedTransactionIds.length === 0 || (!bulkEditVendor && !bulkEditGlAccount)) {
    ...
    try {
      ...
      if (bulkEditVendor) changedFields.push("vendor");
      ...
      for (const id of selectedTransactionIds) {
        ...
        if (originalTx.isLedgerApproved && ( (bulkEditVendor && originalTx.vendor !== bulkEditVendor) || (bulkEditGlAccount && originalTx.glAccount !== bulkEditGlAccount) )) {
            await deleteBankTransactionFromLedger(selectedCompanyId, id, batch);
      ...
      await batch.commit();
      await logAction("BULK_UPDATE_TRANSACTIONS", "BankTransactions", changedFields);
  ...
  const handleConfirmBulkDelete = async () => {
    if (!user || !selectedCompanyId || selectedTransactionIds.length === 0) return;
    setIsLoading(true);
    try {
      const batch = writeBatch(db);
      for (const id of selectedTransactionIds) {
      ...
      await batch.commit();
      await logAction("BULK_DELETE_TRANSACTIONS", "BankTransactions", ["transactions"]);
  ...
  const handlePostSelectedToLedger = async () => {
    if (!user || !selectedCompanyId || selectedTransactionIds.length === 0) {
    ...
    for (const tx of transactionsToAttemptPosting) {
        ...
        //WIP: it will alwyas give error if the Gl Account not start with Bank 
        ...
        if (canPost) {
            ...
            const newRefKeyword = tx.vendor; // Changed this line
            const newRefVendor = tx.vendor;
            const newRefGL = tx.glAccount;

            if (newRefKeyword && newRefKeyword !== '-' && newRefKeyword !== ASK_MY_ACCOUNTANT &&
    ...
    try {
        ...
        if (successfullyPostedIds.length > 0) {
          ...
          await logAction("POST_TO_LEDGER", "BankTransactions", ["ledgerEntries"]);
  ...
  const handleApplyCoaResolutions = async () => {
    ...
    const batch = writeBatch(db);
    const newCoaItems: Omit<PageChartOfAccountItem, 'id'>[] = [];
    ...
    for (const item of missingCoaSuggestionsForDialog) {
      if (item.chosenAction === 'createNew') {
        ...
        const newCoaData: Omit<PageChartOfAccountItem, 'id' > = {
          companyId: selectedCompanyId,
          createdBy: user.uid,
        ...
        // newCoaItemsAddedToDb.push({ ...newCoaData, id: newCoaDocRef.id, createdAt: Timestamp.now() });
    ...
    try {
      ...
      if (newCoaItems.length > 0) {
        await logAction("CREATE_CHART_OF_ACCOUNTS", "BankTransactions", ["glAccounts"]);
        // setChartOfAccounts(prevCoa => [...prevCoa, ...newCoaItems].sort((a, b) => a.glAccount.localeCompare(b.glAccount)));
                    ...
                    {filteredAndSortedTransactions.length > 0 ? (
                      filteredAndSortedTransactions.map((transaction) => (
                        ...
                        >
                          ...
                          <TableCell>{transaction.date ? dateFnsFormat(dateFnsParse(transaction.date, "yyyy-MM-dd", new Date()), "MM/dd/yyyy") : ""}</TableCell>
                ...
                <TableBody>
                  {transactionsForAiConfirmation.map((tx) => (
                    <TableRow key={tx.id}>
                      ...
                      <TableCell className="text-xs">{tx.date ? dateFnsFormat(dateFnsParse(tx.date, "yyyy-MM-dd", new Date()), "MM/dd/yyyy") : ""}</TableCell>
